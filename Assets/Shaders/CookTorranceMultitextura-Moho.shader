Shader "Custom/CookTorranceMultitextura-Moho"
{
    Properties
    {
        ...
        [NoScaleOffset] _SecondaryText ("Texture (Kd)", 2D) = "white" {}
    ...
    SubShader 
    {
        ...
        Pass {
            ...
            fixed4 fragmentShader(v2f f) : SV_Target {
                ...
                fixed4 _MainTex = tex2D(_MainText, f.uv);
                fixed4 _SecondaryTex = tex2D(_SecondaryText, f.uv);
                float3 ambient = _AmbientLight * _AmbientMaterial;

                fixed4 _Color;

                if(_SecondaryTex.g > 0.01f)
                {
                    //_Color = _MainTex;
                }else
                {
                    _Color = _SecondaryTex;
                }
